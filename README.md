...
GL-iNet only ships the client for Windows and macOS. This tool takes the official `.dmg` (or `.exe`) you downloaded, reuses its platform-independent app payload on the Linux Electron runtime, applies the mod, and installs the result as **GLKVM (mod)**, version **V0.1.0 linux · ui-mod 0.1.6** (About says which GLKVM release it is ported from - 1.5.0 release1 today).
...
2. **Apply glikvm-mod's patches verbatim** (`vendor/glikvm-mod/src/patches.ts` + `src/inject/*`), then this repo's Linux patches (`src/patches.mjs`):
   ...
   * version shown as `V0.1.0 linux · ui-mod <ver>` (footer + About title), and an About paragraph saying which GLKVM release it is ported from, linking this repo and stating the non-affiliation - display only, the update-check copy is untouched.
